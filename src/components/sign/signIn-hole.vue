<template>
  <div>
    <!--登录、注册主页面-->
    <div class="sign-page" :class="{signPageIsDisplay: signPageDisplay}">
      <div class="tab">
        <div class="sign-in-txt" :class="{dlTxtIsActive: dlTxtActive}" @click="showSignIn">登录</div>
        <div class="sign-up-txt" :class="{zcTxtIsActive: zcTxtActive}" @click="showSignUp">注册</div>
      </div>

      <div class="sign-in" :class="{dlIsActive: dlActive}">
        <div class="input">
          <chuangkit-input placeholder="邮箱/手机号" name="usr"></chuangkit-input>
          <chuangkit-input placeholder="输入密码" name="pw" type="password"></chuangkit-input>
        </div>

        <span class="forget" @click="showRetrievePage">
        忘记密码
      </span>
      ...
      <div class="sign-up" :class="{zcIsActive: zcActive}">
        <div class="input">
          <chuangkit-input placeholder="输入邮箱或手机号" name="usr"></chuangkit-input>
          <chuangkit-input placeholder="输入图形验证码" name="yzm"></chuangkit-input>
        </div>
        <div class="next-step-button" @click="showTelSignUpPage">
          <button>下一步</button>
        </div>
        <div class="remark">注册代表同意<a href="#" @click="showAgreement">《隐私协议》</a>和<a href="#" @click="showAgreement">《使用条款》</a></div>
        <div class="fast-signup-text">使用社交账号注册</div>
        <div class="fast-signup-icon">
          <span class="sprite weibo"></span>
          <span class="sprite wechat" @click="showWechatQrcode"></span>
          <span class="sprite qq"></span>
        </div>
      </div>
    </div>
    <!--手机号注册页面-->
    <div class="tel-sign-up-page" :class="{telSignUpPageIsDisplay: telSignUpPageDisplay}">
      <div class="tab">
        <div class="sign-in-txt" :class="{dlTxtIsActive: dlTxtActive}" @click="showSignIn">登录</div>
        <div class="sign-up-txt" :class="{zcTxtIsActive: zcTxtActive}" @click="showSignUp">注册</div>
      </div>
      ...
      <div class="sign-up" :class="{zcIsActive: zcActive}">
        <div class="input">
          <chuangkit-input placeholder="输入验证码" name="yzm"></chuangkit-input>
          <chuangkit-input placeholder="设置密码" name="pw"></chuangkit-input>
          <chuangkit-input placeholder="确认密码" name="pw"></chuangkit-input>
        </div>
        <div class="next-step-button" @click="showSignUpSucPage">
          <button>下一步</button>
        </div>
        <div class="get-verify-code">60s后重新发送</div>
      </div>
    </div>
    <!--注册成功页面-->
    <div class="sign-up-success" :class="{signUpSucPageIsDisplay: signUpSucPageDisplay}">
      <div class="sprite success-img"></div>
      <div class="txt">注册成功</div>
      <div class="to-sign-in-button" @click="showSignIn">
        <button>立即登录</button>
      </div>
    </div>
    <!--微信账号注册-->
    <div class="wechat-sign-up-page" :class="{wechatSignUpPageIsDisplay: wechatSignUpPageDisplay}">
      <div class="go-back" @click="showSignUp">
        <span class="sprite go-back-icon"></span><span>返回</span>
      </div>
      <div class="header">微信账号注册</div>
      <div class="qrcode">
        <img src="./QRCode.png" />
      </div>
      <div class="txt1">微信扫描二维码完成注册</div>
      <div class="txt2">注册代表同意<a href="#">《用户协议》</a></div>
    </div>
    <!--找回密码页面-->
    <div class="retrieve-page" :class="{retrievePageIsDisplay: retrievePageDisplay}">
      <div class="go-back" @click="showSignIn">
        <span class="sprite go-back-icon"></span><span>返回</span>
      </div>
      ...
      <div class="tel-retrieve" :class="{telRetriIsActive: telRetriActive}">
        <div class="input">
          <chuangkitInput placeholder="输入手机号" name="tel"></chuangkitInput>
          <chuangkitInput placeholder="输入验证码" name="yzm"></chuangkitInput>
          <chuangkitInput placeholder="输入验证码" name="yzm"></chuangkitInput>
          <span>点击刷新</span>
          <img src="./yzm.png" />
          <div class="get-verify-code">获取短信验证码</div>
        </div>
        <div class="next-step-button" @click="showTelRePw">
          <button>下一步</button>
        </div>
      </div>
      <!--使用邮箱找回-->
      <div class="mail-retrieve" :class="{mailRetriIsActive: mailRetriActive}">
        <div class="input">
          <chuangkitInput placeholder="输入邮箱账号" name="mail"></chuangkitInput>
          <chuangkitInput placeholder="输入图形验证码" name="yzm"></chuangkitInput><img src="./yzm.png" />
        </div>
        <div class="next-step-button" @click="showMailRetriSuc">
          <button>下一步</button>
        </div>
      </div>
    </div>
    <!--使用手机号重置密码页-->
    <div class="tel-re-pw-page" :class="{telRePwPageIsDisplay: telRePwPageDisplay}">
      <div class="go-back" @click="backToRetriPage">
        <span class="sprite go-back-icon"></span><span>返回</span>
      </div>
      <div class="header">重置密码</div>
      <div class="input">
        <chuangkitInput placeholder="输入新密码" name="pw"></chuangkitInput>
        <chuangkitInput placeholder="确认新密码" name="pw"></chuangkitInput>
      </div>
      <div class="next-step-button" @click="showRePwSuccess">
        <button>下一步</button>
      </div>
    </div>
    <!--重置密码成功-->
    <div class="re-pw-success" :class="{rePwSucPageIsDisplay: rePwSucPageDisplay}" >
      <div class="sprite success-img"></div>
    ...
    <!--使用邮箱，发送验证邮件成功-->
    <div class="mail-retrieve-success" :class="{mailRetriSucIsDisplay: mailRetriSucDisplay}">
      <div class="go-back" @click="backToRetriPage">
        <span class="sprite go-back-icon"></span><span>返回</span>
      </div>
      <div class="mail-retrieve-success-content">
        ...
        <div class="success-txt">验证链接已成功发送至<br />
          bingo@holyhilab.com的邮箱</div>
        <div class="to-verify-mail-button" @click="showMailRePwPage">
    ...
    <!--使用邮箱重置密码页-->
    <div class="mail-re-pw-page" :class="{mailRePwPageIsDisplay: mailRePwPageDisplay}">
      <div class="header">重置密码</div>
      <div class="input">
        <chuangkitInput placeholder="输入新密码" name="pw"></chuangkitInput>
        <chuangkitInput placeholder="确认新密码" name="pw"></chuangkitInput>
    ...
    <!--用户协议-->
    <div class="agreement-page" :class="{agreementPageIsDisplay: agreementPageDisplay}">
      <div class="go-back" @click="backToSignPage">
        <span class="sprite go-back-icon"></span><span>返回</span>
      </div>
      ...
      <div class="agreement-contents">
        ...
        <p>7、发送信息到您的电子邮件。</p>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;我们将通过电子邮件的形式，向您告知您的订单、账户等信息，并且您也可以根据您的需要订阅我们的服务。有任何问题可以联系<a href="mailto:service@eyuanku.com">service@chuangkit.com</a></p>
        <p>8、我们会保留您的信息在一段时间中</p>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;您的用户帐户的终止或注销后，我们可以因为相关统计、审计等需要，保留您的个人信息及资料在一段合理时间里。</p>
        <p>9、联系我们</p>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;如果你对隐私协议有任何疑问，都可以联系我们。<a href="mailto:service@eyuanku.com">service@chuangkit.com</a></p>
        <p>10、更新隐私协议</p>
        ...
        <p>三、服务条例</p>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;1.创客贴以任何形式提供的图像、语音、视频及相关资料内容，包括相关的摄影照片、矢量图形、字体、文本、图片说明，或资料信息(以下合称“内容”)，其所有权属于创客贴或其贡献者，并且受中国及国际版权法以及其它知识产权法律法规所保护。创客贴作为网络服务提供者，对非法转载，盗版行为的发生不具备充分的监控能力。但是当版权拥有者提出侵权指控并出示充分的版权证明材料时，请及时发邮件联系我们（<a href="mailto:service@eyuanku.com">service@chuangkit.com</a>），我们将尽快处理。</p>
        ...
        <p>&nbsp;&nbsp;&nbsp;&nbsp;3.如果您使用了未经授权的内容，已触犯《中华人民共和国著作权法》及其他相关法律法规，创客贴将会依据《中华人民共和国著作权法》及其他相关法律法规，行使所有的权利及方法去追讨损失，包括向非法使用内容的人士，以及使用内容的受益者追讨金钱上的赔偿。创客贴会根据此协议、图片使用许可协议的相关条款及有关法律法规，向未经授权者追讨费用、损失及赔偿。除此之外，创客贴还会保留一切权利，向未经授权使用者收取(而未经授权使用者必须在此同意支付)十（10）倍版权费。 再者，创客贴或其贡献者有权就非法使用、违反本协议或图片使用许可协议条款向未经授权使用者采取其它法律或索偿行动。</p>
        ...
        <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;e) 当您因为在社区页面引用本网站内容而引起任何第三方索赔和要求时。您同意对创客贴及其内容资源，管理人员、董事、雇员、承包商、子公司、合资企业、授权商和被授权商进行赔偿，并保证其免受任何损害，并承担所有索赔(包括但不限于第三方索赔)，赔偿责任、造成的损失(包括惩罚性损害赔偿)、判决、协议、费用及开支，当中包括合理的法律费用及开支。</p>
        <p>七、协议终止</p>
        ...
        <p>&nbsp;&nbsp;&nbsp;&nbsp;4、暂停或终止您的账户，不另行通知您。如果发现或创客贴有证据认为您有违反一次使用协议或是任何滥用你的用户名和密码的行为，我们将终止您的账号，并没收所有已得的付费。</p>
        <p>三、一次使用协议条款</p>
        ...
        <p>2.创客贴不会就任何内容，包括产品、服务及软件作出任何承诺。在不与法律抵触的原则下创客贴拒绝对有关内容(包括产品、服务和软件)，包括但不限于内容的准确性、及时性、完整性、时效性、可用性、有效性、非侵权性、安全性、保密性、所有权、有用性、适用性、质量、适销性、特定用途的适用性，作出任何明示或暗示性的保证。此外，创客贴不作任何陈述，担保或保证，证明有关内容(包括产品，服务和软件)符合您的需要，也不能确保内容可以不受干扰，及时提供、并准确、安全地使用，又或不存在任何错误或缺陷。</p>
        <p>3.除法律不能豁免的责任外，创客贴或其任何管理人员、董事、雇员、附属公司、继任者、母公司、合资公司、关联机构、代理、承包商、内容提供者、供应商、授权商或被授权商及其各自的管理人员、董事、雇员(以下全部统称“创客贴相关方”)，对您或通过您使用本网站或其内容的第三方，均不会承担任何因为使用或有关使用或无法使用本网站或其内容，包括病毒、未能传送讯息、数据损坏、传输错误，或因使用互联网服务供应商，或因链接到第三方网站和第三方网站内容，而导致的任何损害，包括但不限于利润的损失、机会的损失、间接的、附带的、特殊的、惩罚性的、惩戒性的、严重的、法定的、经济的或必然的损害赔偿。</p>
        <p>4.因为任何理由（包括但不限于合同违反、侵权、严重疏忽，或以其它方式），令您可能遭受任何损害或损失，创客贴及创客贴相关方的累计赔偿总额(如适用)，应只限于在损害或损失发生前您向创客贴所支付的金额。在任何情况下，因使用本网站而引致索赔，创客贴最高累计赔偿总额不会超过壹佰元（100元）人民币。</p>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;更新于：2014年12月11日</p>
      </div>
      <div class="agree-button" @click="showSignUp">
        <button>同意</button>
      </div>
    </div>
...
<script>
  import chuangkitInput from '../input/input.vue'
  ...
  export default {
    ...
    data(){
      return {
        ...
        agreementPageDisplay: false,
        telRePwPageDisplay: false,
        rePwSucPageDisplay: false,
        mailRePwPageDisplay: false,
        wechatSignUpPageDisplay: false,
        telSignUpPageDisplay: false,
        signUpSucPageDisplay: false
      }
    },
    methods: {
      showSignIn() {
        this.signPageDisplay = true;
        this.dlTxtActive = true;
        this.dlActive = true;
        this.zcTxtActive = false;
        this.zcActive = false;
        this.mailRetriSucDisplay = false;
        this.retrievePageDisplay = false;
        this.rePwSucPageDisplay = false;
        this.signUpSucPageDisplay = false;
      },
      showSignUp() {
        this.signPageDisplay = true;
        this.dlTxtActive = false;
        this.dlActive = false;
        this.zcTxtActive = true;
        this.zcActive = true;
        this.wechatSignUpPageDisplay = false;
        this.agreementPageDisplay = false;
      },
      showRetrievePage() {
        this.signPageDisplay = false;
      ...
      showAgreement(){
        this.signPageDisplay = false;
        this.agreementPageDisplay = true;
      },
      showMailRetriSuc(){
        this.retrievePageDisplay = false;
        this.mailRetriSucDisplay = true;
      },
      showTelRePw(){
        this.retrievePageDisplay = false;
      ...
      showRePwSuccess(){
        ...
        this.telRePwPageDisplay = false;
        this.mailRePwPageDisplay = false
      },
      showMailRePwPage(){
        this.mailRetriSucDisplay = false;
        this.mailRePwPageDisplay = true;
      },
      showWechatQrcode(){
        this.signPageDisplay = false;
        this.wechatSignUpPageDisplay = true;
      },
      showTelSignUpPage(){
        this.signPageDisplay = false;
        this.telSignUpPageDisplay = true;
        this.zcTxtActive = true;
        this.dlTxtActive = false;
      },
      showSignUpSucPage(){
        this.telSignUpPageDisplay = false;
        this.signUpSucPageDisplay = true;
      }
    },
...
<style lang="less" scoped>
  ...
  .dlTxtIsActive,
  .mailRetriTxtIsActive,
  .telRetriTxtIsActive,
  .zcTxtIsActive {
    color: #626262;
  }

  .agreementPageIsDisplay,
  .dlIsActive,
  .mailRetriIsActive,
  .retrievePageIsDisplay,
  .signPageIsDisplay,
  .telRetriIsActive,
  .zcIsActive,
  .mailRetriSucIsDisplay,
  .telRePwPageIsDisplay,
  .rePwSucPageIsDisplay,
  .mailRePwPageIsDisplay,
  .wechatSignUpPageIsDisplay,
  .telSignUpPageIsDisplay,
  .signUpSucPageIsDisplay
  {
    display: block;
  }
