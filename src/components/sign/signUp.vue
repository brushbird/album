  ...
  <div id="sign-up-page">
    <div class="sign-up-page">
      ...
      <div class="sign-up-first" v-if="signUpFirstDisplay">
        <div class="tab">
          <div class="sign-in-txt" :class="{dlTxtIsActive: dlTxtActive}">
            <router-link to="/signin">登录</router-link>
          </div>
          <div class="sign-up-txt" :class="{zcTxtIsActive: zcTxtActive}">
        ...
        <div class="sign-up" :class="{zcIsActive: zcActive}">
          ...
          <div class="remark">注册代表同意<a @click="showAgreement">《隐私协议》</a>和<a @click="showAgreement">《使用条款》</a>
          </div>
      ...
      <!--手机号/邮箱注册页面-->
      <div class="tel-sign-up-page" v-if="nextSignUpPageDisplay">
        <div class="tab">
        ...
        <div class="sign-up" :class="{zcIsActive: zcActive}">
          <div class="input">
            ...
            <chuangkit-input
                    v-if="telSignUp"
                    @inputResult="checkInputState(arguments,'mailSignUpYzm')"
                    placeholder="输入验证码"
                    name="yzm"
                    width="200"
                    boxWidth="350"
            ...
            <!--手机注册页专有-->
            <chuangkit-input
                    @inputResult="checkInputState(arguments,'mailSignUpPw1')"
            ...
            <chuangkit-input
                    @inputResult="checkInputState(arguments,'mailSignUpPw2')"
      ...
      <!--注册成功页面-->
      <div class="sign-up-success" v-if="signUpSucDisplay">
      ...
      <!--微信账号注册-->
      <div class="wechat-sign-up-page" v-if="wechatSignUpPageDisplay">
        ...
        <div class="txt2">注册代表同意<a @click="showAgreement">《用户协议》</a></div>
      </div>
      <!--用户协议-->
  ...
  export default {
    data(){
      return {
        ...
        zcTxtActive: true,
        zcActive: true,
        signUpFirstDisplay: true,
        agreementPageDisplay: false,
        nextSignUpPageDisplay: false,
        signUpSucDisplay: false,
        wechatSignUpPageDisplay: false,
        nextBtnActive: {
          nextBtnActived: false
        },
        signUpNextBtn: {
          signUpNextBtnActived: false
        }
      }
    },
    methods: {
      showSignIn(){
        this.signUpSucDisplay = false;
      },
      showWechatQrcode(){
      ...
      telOrMail(val){
        let mail = /^[a-z_0-9.-]{1,64}@([a-z0-9-]{1,200}.){1,5}[a-z]{1,6}$/;
        // 根据输入框的数据判断是否是邮箱格式，若非，则显示手机专有验证码框
        this.telSignUp = mail.test(val) ? false : true;
      },
      inputValue(val){
        if(val){
          this.firstPwd = "";
      ...
      checkInputState(result,val){
        ...
        switch (val){
          case 'signUpUsr': this.signUpUsrFilled = state !== "error" ? true : false;
            break;
          case 'signUpYzm': this.signUpYzmFilled = state == "success" ? true : false;
            break;
          case 'mailSignUpYzm': this.mailSignUpYzmFilled = state == "success" ? true : false;
            break;
          case 'mailSignUpPw1': this.mailSignUpPwd1Filled = state == "success" ? true : false;
            break;
          case 'mailSignUpPw2': this.mailSignUpPwd2Filled = state == "success" ? true : false;
            break;
        }
      ...
      inputState(){
        ...
        this.nextBtnActive.nextBtnActived = this.signUpUsrFilled && this.signUpYzmFilled ? true : false;
        // 若两个密码均已填入且一致
        let pwdAreOk = this.mailSignUpPwd1Filled && this.mailSignUpPwd2Filled && this.uniPwd;
        if(!this.telSignUp){ //填入的是邮箱
          this.signUpNextBtn.signUpNextBtnActived = pwdAreOk ? true : false;
        }
        else{//填入的是手机号，增加验证码 input 并检测
          this.signUpNextBtn.signUpNextBtnActived = pwdAreOk && this.mailSignUpYzmFilled ? true : false;
        }
      },
      checkSignUp() {
        this.$http.get('mock/user/reg').then(response => {
          let signUpResult = response.body.code;
          this.signUpSucDisplay = signUpResult == "1" ? true : false;
          this.nextSignUpPageDisplay = false;
          console.log("请求了！");
        }, response => {
      ...
      showNextSignUpPage(){
        this.$http.get('mock/user/sendResetPwdEmail').then(response => {
          ...
          this.nextSignUpPageDisplay = sendResult == "1" ? true : false;
          this.signUpFirstDisplay = false;
          console.log("请求了！");
        }, response => {
          console.log('Signup error');
